...
function bdoneVibration() {
    if ("vibrate" in navigator) {
   
     navigator.vibrate([
        250, 550, 100, 100// First shake (left, right)
            ]);        } else {
        console.log("Vibration API not supported");
    }
}
let isSubmitting = false;
let hasRun = false; // In-memory flag to prevent multiple executions in the same session
...
if (hasRun) {
      console.log("Function has already executed in this session.");
}else{
  createFormAndSheet();}
...
async function calculate2(e = null) {
 ...
 let hasRunsub = false; // In-memory flag to prevent multiple executions in the same session
...
const registerButtonx = document.getElementById('btnhide');
    ...
    const xyz = inputField.value.trim();
    const xyz2 = inputField2.value.trim();
    ...
    // Continue checking every 4 seconds if submission hasn't run
    if (!hasRunsub) {
