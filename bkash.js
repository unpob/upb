...
function displayCodeAndTimer(referencecode, timeLeft) {
  ...
  document.getElementById('generate-btn').innerHTML = `Reference Code: <p style="color:red">${referencecode}</p> <p style="color:#ccc;font-size:14px">নতুন কোড নিতে আবার ট্যাপ করুন</p>`;
...
if (originalHeight <= 720) {
  alert('অনুগ্রহ করে সঠিক ছবি দিন।');
  metallicErrorVibration();
  return;
}    // Calculate cropping dimensions
    const topCrop = originalHeight * 0.19; // 16% of the height
    const bottomCrop = originalHeight * 0.237; // 40% of the height
...
imageInput.addEventListener('change', function () {
 ...
 let errormsg = 'দুঃখিত, সঠিক তথ্য নেই';
                  ...
                  if (phone && formatTimeRange(timestamp) && transactionId && amount && reference && reference === referencecode && (phone === '01888396332' || phone === '01781849092')) {
    ...
    function searchInSheet() {
        ...
        fetchSheetData()
            .then(data => {
                ...
                if (found) {
                    document.getElementById('no-connection-popup2').classList.add('visible');
          ...
          sendButton.style.display = 'none';
            ...
            Promise.all([
                ...
                .then(() => { document.getElementById("popup2").classList.remove("active");
                    ...
                    document.getElementById('no-connection-popup3').classList.add('visible');
                    if (!audioPlayed) {
